...
## Why this matters
You have stared at this app for 100 hours. You know exactly what to click. A new user does not. Beta testing exposes the incredibly obvious UX flaws that you were completely blind to because you built the system.
...
### Production SaaS
Invite 10-20 "Friendly" users (people in your network or niche communities). Watch them use the app live over Zoom if possible. Do not explain what to do—just watch where they get stuck. Use tools like PostHog Session Replays or Hotjar to anonymously record how early users navigate the site to identify dead ends or broken buttons.

## Implementation Steps
\`\`\`prompt
Act as a UX Researcher. I am preparing to launch the private beta for my SaaS app. Write a short email template I can send to 20 potential beta testers inviting them to try the app. Also, provide a list of 5 questions I should ask them after their first session to extract the most valuable feedback.
\`\`\`

## Validation Checklist
- [ ] At least 5 people have used the app who did not build it.
- [ ] You have fixed any "Showstopper" bugs discovered during beta testing.
- [ ] You have session recording enabled (e.g., PostHog/Hotjar) for the beta period.
\`,`,
...
## Why this matters
If users hit a bug and have no easy way to tell you, they will simply leave. A frictionless feedback loop is the lifeblood of early-stage software.
...
### Production SaaS
Add an in-app feedback widget. This could be a floating Chat bubble (Intercom/Crisp), a simple "Send Feedback" modal that hits an API endpoint and sends you an email, or a Canny.io board. If users are confused, they should be able to reach you in 2 clicks.

## Implementation Steps
\`\`\`prompt
Act as a Product Manager. I want to add a lightweight feedback widget to my React web app without paying $100/mo for Intercom. Recommend 3 free or low-cost tools that integrate well with React, or walk me through building a custom "Submit Feedback" form that sends a message to my Slack channel via a webhook.
\`\`\`

## Validation Checklist
- [ ] Users can submit feedback without leaving the app.
- [ ] Feedback submissions send an instant alert to the founders (Email/Slack/Discord).
\`,`,
...
## Why this matters
\`my-app-v2-final.vercel.app\` does not inspire trust. You need a professional \`.com\`, \`.io\`, or \`.co\` domain.
...
### Production SaaS
Buy a domain from Cloudflare or Namecheap. Update the nameservers to point to your hosting provider (e.g., Vercel). Ensure SSL certificates are automatically provisioned. If you are sending emails from this domain, this is also when you add the DKIM/SPF TXT records to ensure your emails avoid the spam folder.

## Implementation Steps
\`\`\`prompt
Act as a DevOps Engineer. I just bought a domain on Cloudflare and I am hosting my frontend on [Insert Provider, e.g., Vercel]. Walk me through the exact DNS records (A, CNAME) I need to configure to point my root domain and \`www\` subdomain to my hosting provider.
\`\`\`

## Validation Checklist
- [ ] Your app is accessible via your custom domain.
- [ ] The browser displays a secure HTTPS padlock.
- [ ] \`www.yourdomain.com\` correctly redirects to \`yourdomain.com\` (or vice versa).
\`,`,
  'webhostingdeploy': `# Production Hosting
...
## Why this matters
The environment where you run your app locally (\`localhost:3000\`) is wildly different from a production environment. Production hosting requires zero downtime deployments, automatic scaling, and secure environment variable injection.
...
### Production SaaS
If your app is mostly frontend or uses Serverless (Next.js), Vercel is the easiest, though potentially expensive at scale. If you have a custom Node.js/Python backend, use Render, Fly.io, or AWS ECS. Ensure your production database is hosted in the *exact same geographic region* as your backend server to eliminate latency (e.g., both in \`us-east-1\`).

## Implementation Steps
\`\`\`prompt
Act as a Cloud Architect. I am deploying a [Insert Stack] app for production. My primary users are located in [Insert Region, e.g., Europe]. Recommend the best hosting provider, explain how to deploy my code via a GitHub integration, and highlight how to securely manage my production \`.env\` variables.
\`\`\`

## Validation Checklist
- [ ] The backend and database are in the same AWS/GCP region.
- [ ] Deployments trigger automatically when merging to the \`main\` branch.
- [ ] You have verified that production API keys are securely set in the dashboard.
\`,`,
...
## Why this matters
You cannot improve what you do not measure. If you launch without analytics, you are flying blind. You won't know if your marketing is working or if users are dropping off at the signup screen.
...
### Production SaaS
Install Google Analytics 4 (GA4) for marketing attribution (where did the user come from?) and PostHog/Mixpanel for product analytics (what did the user do inside the app?). Crucially, track the "Aha!" moment—the specific action that proves the user got value out of your app.

## Implementation Steps
\`\`\`prompt
Act as a Data Analyst. I am launching a SaaS app. I want to track marketing visits with Google Analytics, and in-app behavior with PostHog. Provide the initialization scripts for both, and explain how to track a custom "User Upgraded" event in PostHog when someone purchases a subscription.
\`\`\`

## Validation Checklist
- [ ] Analytics scripts are live on production.
- [ ] You have excluded your own IP address so your testing doesn't skew the data.
- [ ] Custom events for core actions (Sign Up, Purchase, Core Feature Use) are firing.
\`,`,
...
## Why this matters
If you process payments or collect email addresses, you are legally required to have a Privacy Policy and Terms of Service. Payment processors (like Stripe) will suspend your account if your website does not have these pages publicly linked in the footer.
...
### Production SaaS
Do not pay a lawyer $5,000 for a pre-launch SaaS. Use a free generator like Termly or generate a basic template via an LLM. Ensure you explicitly state how user data is handled, especially if you are using AI (e.g., "We do not use customer data to train public models"). Place the links in your site footer.

## Implementation Steps
\`\`\`prompt
Act as a Legal Advisor for Tech Startups. I am launching a B2B SaaS that [Insert brief description of what it does and what data it collects]. Generate a standard 'Terms of Service' and 'Privacy Policy' template that I can adapt for my website. Highlight the sections where I need to insert my company name and email.
\`\`\`

## Validation Checklist
- [ ] Privacy Policy is published and linked in the footer.
- [ ] Terms of Service is published and linked in the footer.
- [ ] Stripe (or your payment processor) has verified your website.
\`,`,
...
## Why this matters
Launch day is chaotic. If you don't have a checklist, you will forget to change the Stripe API keys from "Test" to "Live", or you'll launch with \`console.log("TESTING")\` printing sensitive data in the browser.
...
### Production SaaS
Run a final End-to-End manual test in the production environment. Create a real account, pay with a real credit card, and use the core feature. Check that emails actually arrive. Ensure the database backups are active. Once verified, post your launch on Product Hunt, Hacker News, X, and LinkedIn.

## Implementation Steps
\`\`\`prompt
Act as a SaaS Founder. I am launching my app tomorrow. Provide a comprehensive "Day Before Launch" technical checklist. Include critical checks for Stripe keys, email delivery, environment variables, and database security.
\`\`\`

## Validation Checklist
- [ ] Stripe/Payment keys are set to "Live".
- [ ] You have successfully completed a test purchase in production.
- [ ] You have prepared your marketing assets (Screenshots, Product Hunt copy).
\`,`,
  'webanalytics': `# Advanced Analytics
...
## Why this matters
Basic page views are useless once you have real users. You need to understand cohorts, churn rates, and feature adoption. If you don't track metrics, you are just guessing what to build next.
...
### Production SaaS
Set up funnels in PostHog or Mixpanel. Example: Landing Page View -> Signup -> First Action -> Upgraded. Identify exactly where users are dropping off. Track the "Time to Value" (how long does it take a user to experience the core benefit of the app?).

## Implementation Steps
\`\`\`prompt
Act as a Data Scientist. I have implemented PostHog in my SaaS. My core user flow is: 1. Visit Landing Page, 2. Sign Up, 3. Create a Project, 4. Upgrade to Paid. Explain how to build a funnel report for this flow, and how I can analyze "Drop-off" at step 3 to improve my onboarding UX.
\`\`\`

## Validation Checklist
- [ ] You have a funnel report set up for your primary conversion flow.
- [ ] You track Monthly Recurring Revenue (MRR) and Churn Rate accurately.
\`,`,
...
## Why this matters
As your database grows from 10,000 rows to 10,000,000 rows, queries that used to take 10ms will start taking 5 seconds. The server will run out of memory. You need a proactive strategy to handle scale before it takes your app offline.
...
### Production SaaS
Monitor slow queries in your PostgreSQL database and add Indexes to frequently filtered columns. Implement read-replicas to offload heavy \`SELECT\` queries from your primary database. Move heavy, long-running tasks (like PDF generation or bulk email sending) off the main API thread and into background workers (using BullMQ, AWS SQS, or trigger.dev).

## Implementation Steps
\`\`\`prompt
Act as a Cloud Architect. My Node.js app is experiencing slow response times because a specific database query is taking 3 seconds. Explain how to use \`EXPLAIN ANALYZE\` in PostgreSQL to identify missing indexes, and walk me through setting up a background worker queue (e.g., using BullMQ) to offload heavy tasks.
\`\`\`

## Validation Checklist
- [ ] Slow database queries have been indexed.
- [ ] Long-running tasks are handled asynchronously via queues.
- [ ] Database connection pooling is optimized for high traffic.
\`,`,
...
## Why this matters
Organic traffic is the holy grail of SaaS growth. To capture it, you need to rank for the keywords your target audience is searching for.
...
### Production SaaS
Start a programmatic SEO strategy or a high-quality blog. Use Next.js to generate thousands of template pages based on database records (e.g., "Best CRM for [Industry]"). Ensure your Core Web Vitals are green, as Google penalizes slow, clunky websites. Build backlinks by launching free side-tools related to your niche.

## Implementation Steps
\`\`\`prompt
Act as an SEO Marketer. I run a SaaS that does [Insert App Function]. Suggest 10 high-intent, low-competition long-tail keywords I should target. Then, explain the concept of "Programmatic SEO" and how I can use Next.js dynamic routes to capture thousands of search queries.
\`\`\`

## Validation Checklist
- [ ] You are producing content targeting specific search keywords.
- [ ] Your Core Web Vitals are passing Google's assessments.
- [ ] You have a strategy for acquiring high-quality backlinks.
\`,`,
...
## Why this matters
Acquiring a new user is 5x more expensive than keeping an existing one. If your churn rate is 20% per month, your SaaS will die, regardless of how good your marketing is.
...
### Production SaaS
Implement lifecycle emails. If a user signs up but doesn't create a project within 3 days, send an automated email offering help. Build a "Cancellation Flow" that asks users why they are leaving (and perhaps offers a 50% discount for the next month to save them). Improve the initial onboarding experience.

## Implementation Steps
\`\`\`prompt
Act as a Product Manager. My SaaS app has a high churn rate (users cancel after 1 month). Design a "Drip Email Campaign" specifically for onboarding new users over their first 14 days to ensure they reach the "Aha!" moment. What should each email say?
\`\`\`

## Validation Checklist
- [ ] You have automated onboarding emails set up.
- [ ] You require users to provide a reason when they cancel their subscription.
- [ ] You actively reach out to power users to build relationships.
\`,`,
...
## Why this matters
You will receive 100 feature requests from users. If you build them all, your app will become a bloated, unusable mess. You need a system for prioritizing what to build.
...
### Production SaaS
Use the RICE scoring model (Reach, Impact, Confidence, Effort) to prioritize features. Maintain a public roadmap (using Canny or Notion) so users can vote on features and see that the product is actively maintained. Say "No" to features that only 1% of your user base wants, even if they pay you.

## Implementation Steps
\`\`\`prompt
Act as a Lead Product Manager. I have 10 competing feature requests from my users. Explain the RICE scoring framework. Then, act as my sounding board: I will give you 3 feature ideas, and you will help me score them to decide what my engineering team should build next.
\`\`\`

## Validation Checklist
- [ ] You have a public (or internal) roadmap of planned features.
- [ ] You use a framework (like RICE) to prioritize work over gut feeling.
- [ ] You regularly ship updates and communicate them via a Changelog.
\`,`,
  'webtechnicaldebt': `# Managing Technical Debt
...
## Why this matters
To launch fast, you wrote messy code. That was the right decision. But now, adding a new feature takes 3 weeks instead of 3 days because the codebase is so fragile.
...
### Production SaaS
Dedicate 20% of every engineering cycle strictly to refactoring and paying down technical debt. Upgrade outdated npm packages to avoid security vulnerabilities. Refactor massive 1,000-line React components into smaller, testable pieces. Improve test coverage on critical billing and auth flows.

## Implementation Steps
\`\`\`prompt
Act as a Senior Staff Engineer. My React/Node codebase has become very fragile. We have massive files, duplicate logic, and no tests. Give me a strategic, step-by-step plan for how to safely refactor this legacy code over the next 3 months without halting all new feature development.
\`\`\`

## Validation Checklist
- [ ] You regularly update your dependencies (\`npm outdated\`).
- [ ] You dedicate a percentage of your time to refactoring rather than just shipping features.
- [ ] Critical paths have high automated test coverage.
\`,`
};
