...
linkGroups.forEach(group => {
  ...
  group.keys.forEach(key => {
    // Just find the key definition
    const startPattern = `'${key}': `;
    const startIndex = content.indexOf(startPattern);
    
    if (startIndex !== -1) {
      // Find the next comma that marks the end of this key's value
      // A safe way is to find the next key pattern e.g. "  '" or "}"
      // Actually we know the value ends with "`, " or "\`,"
      let nextComma = content.indexOf('\`,', startIndex);
      let nextComma2 = content.indexOf('\`,\n', startIndex);
      
      // Let's just find the last backtick before the next key
      const nextKeyStart = content.indexOf(`  '`, startIndex + startPattern.length);
      const endOfValue = nextKeyStart !== -1 ? nextKeyStart : content.lastIndexOf('}');
      
      const block = content.substring(startIndex, endOfValue);
      
      if (!block.includes('📚 Context Links')) {
        // The block looks like: `'key': \`...content...\`,\n`
        // We want to insert linksMarkdown before the closing `
        const lastBacktick = block.lastIndexOf('\`');
        if (lastBacktick !== -1) {
           const newBlock = block.substring(0, lastBacktick) + linksMarkdown + block.substring(lastBacktick);
           content = content.substring(0, startIndex) + newBlock + content.substring(endOfValue);
           updatedCount++;
        }
      } else {
