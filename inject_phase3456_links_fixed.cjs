...
linkGroups.forEach(group => {
  ...
  group.keys.forEach(key => {
    // Avoid regex escaping bugs by splitting
    const searchString = `'${key}': \``;
    const parts = content.split(searchString);
    
    if (parts.length === 2) {
      // Find the ending backtick and comma
      const endingIndex = parts[1].indexOf('\`,');
      if (endingIndex !== -1) {
        const textInside = parts[1].substring(0, endingIndex);
        if (!textInside.includes('📚 Context Links')) {
          parts[1] = textInside + linksMarkdown + parts[1].substring(endingIndex);
        ...
        console.log(`WARNING: Key ${key} found but no ending backtick-comma.`);
      }
    } else {
      console.log(`WARNING: Key ${key} not found exactly once. Parts: ${parts.length}`);
    }
  });
});
